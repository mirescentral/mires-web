...
export default async function AdminMultimediaPage({
  ...
  searchParams: Promise<{ success?: string, error?: string, actualizado?: string }>;
}) {
  // Esperamos la resolución de los parámetros de la URL de forma segura
  ...
  return (
    <div className="min-h-screen bg-[#F9F8F6] text-[#0A0A0A] p-8 md:p-12 font-sans max-w-5xl mx-auto pt-32">
      ...
      {/* Uso seguro de params resueltos */}
      {params?.success && (
        <div className="mb-10 p-5 bg-green-50 border border-green-200 text-green-800 text-xs font-semibold tracking-wide uppercase text-center">
          ✓ La imagen para la sección '{params.actualizado}' fue actualizada e implementada en vivo con éxito.
        </div>
      )}

      {params?.error === 'sin_archivo' && (
